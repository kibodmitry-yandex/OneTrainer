...
SETTINGS_PATH = Path(__file__).resolve().parents[2] / 'workspace' / 'masking_tool_settings.json'
...
class VideoParserWindow(ctk.CTkToplevel):
            ...
            try:
                ...
                try:
                    ...
                    try:
                        ...
                        try:
                            logger.debug(f"[parsing_sidecar] atomic write failed (replace): {perr}")
                        except Exception:
                            pass
            finally:
                try:
    ...
    def _merge_and_write_sidecar(self, out_dir: Path, new_files_meta: list, selected_name: str | None):
    ...
    def _validate_and_fix_crop(self, image_w: int, image_h: int, crop: dict) -> dict:
        ...
        size_px_allowed = image_h if orientation == 'horizontal' else (image_w if orientation == 'vertical' else image_w)
        # Read current
        try:
    ...
    # Top panel for file operations
        top_panel = ctk.CTkFrame(self, corner_radius=8, border_width=2, border_color="#444")
        ...
        self.open_btn = ctk.CTkButton(controls_frame, text="Open", width=80, command=self._on_open_file)
        self.open_btn.grid(row=0, column=0, padx=(0,8), pady=4)

        self.file_path_var = ctk.StringVar(value=self._load_setting('video_last_file', '(no file)'))
        self.file_path_label = ctk.CTkLabel(controls_frame, textvariable=self.file_path_var, anchor='w', cursor='hand2')
    ...
    # Clickable only when a file is selected
        ...
        self.every_n_var = ctk.StringVar(value=str(self._load_setting('video_every_n', 10)))
        self.every_n_entry = ctk.CTkEntry(controls_frame, width=80, textvariable=self.every_n_var)
        ...
        self.dedup_chk = ctk.CTkCheckBox(controls_frame, text='Deduplication', variable=self.dedup_var, command=self._on_dedup_toggle)
        ...
        def show_dedup_threshold_dialog():
            ...
            entry.focus_set()
        help_btn = ctk.CTkButton(controls_frame, text='Threshold', width=80, command=show_dedup_threshold_dialog)
        help_btn.grid(row=0, column=5, padx=(0,8))

        self.dedup_threshold_entry = ctk.CTkEntry(controls_frame, width=60, textvariable=self.dedup_threshold_var)
        ...
        try:
            ...
            pass
        try:
            self.dedup_threshold_entry.bind('<FocusOut>', lambda e: self._on_dedup_threshold_change())
            self.dedup_threshold_entry.bind('<Return>', lambda e: self._on_dedup_threshold_change())
        except Exception:
            pass
    ...
    # Dataset clear button
        ...
        try:
            ...
            def show_tooltip(event):
                try:
                    ...
                    y = event.y_root + 10
                    win.geometry(f'+{x}+{y}')
                    label = tk.Label(win, text='Clear dataset', bg='#fff', fg='#a00', relief='solid', borderwidth=1, font=('Arial', 10))
                    label.pack()
                    tooltip_win[0] = win
                except Exception:
                    pass
        ...
        self.run_btn = ctk.CTkButton(controls_frame, text='Run', width=80, command=self._on_run)
        self.run_btn.grid(row=0, column=7, padx=(0,8))

    # Bottom part: horizontal split (bottom panel)
        bottom_panel = ctk.CTkFrame(self, corner_radius=8, border_width=2, border_color="#444")
        ...
        # File navigator (left)
        self.nav_frame = ctk.CTkFrame(bottom_panel, corner_radius=6, border_width=2, border_color="#666")
        ...
        nav_label = ctk.CTkLabel(self.nav_frame, text="File Navigator", font=("Arial", 14, "bold"))
        ...
        try:
            self.file_listbox.bind('<<ListboxSelect>>', lambda e: self._on_listbox_select(e))
        except Exception:
            pass
        ...
        # Debounce delay for navigator updates (ms). Large datasets make
        # frequent updates expensive, so we coalesce updates from the worker.
        self._nav_debounce_ms = 5000

        # Preview (right)
        preview_frame = ctk.CTkFrame(bottom_panel, corner_radius=6, border_width=2, border_color="#666")
        ...
        preview_label = ctk.CTkLabel(header_frame, text="Preview", font=("Arial", 14, "bold"))
        preview_label.grid(row=0, column=0, sticky="w")

        # Frame size selector (square sizes for SD and other models)
        sizes = ["256x256", "384x384", "512x512", "640x640", "768x768", "1024x1024", "1280x1280"]
        ...
        try:
            self.frame_size_menu = ctk.CTkOptionMenu(header_frame, values=sizes, variable=self.video_frame_size_var, command=_on_frame_size_change)
            self.frame_size_menu.grid(row=0, column=1, sticky='e')
        except Exception:
            # fallback: use normal OptionMenu if CTkOptionMenu unavailable
            try:
                import tkinter as _tk
                self.frame_size_menu = _tk.OptionMenu(header_frame, self.video_frame_size_var, *sizes, command=_on_frame_size_change)
                self.frame_size_menu.grid(row=0, column=1, sticky='e')
            except Exception:
                pass
        ...
        import tkinter as _tk
        self.preview_canvas = _tk.Canvas(preview_frame, bg='#2a2a2a', highlightthickness=0)
        ...
        self._drag_anchor = None     # (anchor_x, anchor_y) in image px relative to crop top-left
        # Mouse bindings
        ...
        self.preview_meta_label = ctk.CTkLabel(preview_frame, text="", anchor='w', justify='left')
        ...
        try:
            ...
            self.preview_canvas.bind('<Configure>', lambda e: self._update_preview_image())
        except Exception:
            pass
        ...
        status_label = ctk.CTkLabel(status_frame, textvariable=self.status_var, anchor='w', font=('Consolas', 12), text_color='#eee')
        status_label.pack(fill='x', padx=12, pady=2)
    ...
    def _update_preview_image(self):
        ...
        try:
            ...
            try:
                ...
                try:
                    self._disp_img_id = self.preview_canvas.create_image(self._disp_x0, self._disp_y0, anchor='nw', image=self._disp_photo, tags=('preview_img',))
                    ...
                    self._disp_img_id = self.preview_canvas.create_image(self._disp_x0, self._disp_y0, anchor='nw', image=self._disp_photo)
            except Exception:
                self._disp_photo = None
            ...
            if not crop:
                ...
                crop = {'image_w': img.width, 'image_h': img.height, 'size_px': size_px, 'x_px': x_px, 'y_px': y_px}
            ...
            try:
                ...
                if not self._crop or int(self._crop.get('size_px', 0)) <= 0:
                    self._crop = self._validate_and_fix_crop(img.width, img.height, self._crop or {})
            ...
            try:
                ...
                chosen_size = self.video_frame_size_var.get() if getattr(self, 'video_frame_size_var', None) else ''
                text = f"{sel}\n{dims} — {size}"
                if chosen_size:
    ...
    def _draw_cropper(self):
        ...
        try:
            ...
            try:
                self._crop_rect_id = self.preview_canvas.create_rectangle(x1, y1, x2, y2, outline='#ffa500', width=3, fill='', tags=('cropper',))
            except Exception:
                # fallback without tags
                try:
                    self._crop_rect_id = self.preview_canvas.create_rectangle(x1, y1, x2, y2, outline='#ffa500', width=3, fill='')
                except Exception:
                    self._crop_rect_id = None
    ...
    def _on_canvas_click(self, event):
        ...
        self._drag_axis = 'x' if orient == 'horizontal' else ('y' if orient == 'vertical' else None)
        self._dragging = True
        ...
        try:
            ...
            try:
                self._set_status(f"Crop offset: x={x_px} y={y_px} (norm={x_norm:.3f},{y_norm:.3f})")
            except Exception:
                pass
        except Exception:
            pass
            ...
            for i in range(self.file_listbox.size()):
                ...
                try:
                    ...
                    try:
                        ...
                        self.file_listbox.configure(selectbackground=bg, selectforeground=fg)
                    except Exception:
                        pass
        except Exception:
            pass
    ...
    def _schedule_nav_update(self, delay_ms: int | None = None):
        """Debounced schedule for _update_file_navigator.

        Cancel any pending scheduled navigator update and schedule a new one
        after delay_ms milliseconds. If delay_ms is None, uses
        self._nav_debounce_ms.
        """
        try:
            if delay_ms is None:
                delay_ms = getattr(self, '_nav_debounce_ms', 5000)
            # cancel previous job if any (only call after_cancel with non-None id)
            try:
                ...
                pass
            # schedule new job (convert delay to int safely)
            try:
                delay_val = int(delay_ms)
                self._nav_update_job = self.after(delay_val, lambda: self._update_file_navigator(force=True))
            except Exception:
    ...
    def _on_run(self):
        ...
        logger = logging.getLogger('video_parser')
        logger.info(f"Run clicked: file={self.file_path_var.get()}, every_n={self.every_n_var.get()}, dedup={self.dedup_var.get()}")
        ...
        def _find_ffmpeg_executable():
            ...
            if venv:
                candidate = Path(venv) / ('Scripts' if os.name == 'nt' else 'bin') / ('ffmpeg.exe' if os.name == 'nt' else 'ffmpeg')
                if candidate.exists():
                    return str(candidate)
            # also check next to python executable
            try:
                pyexe = Path(sys.executable)
                candidate = pyexe.parent / ('ffmpeg.exe' if os.name == 'nt' else 'ffmpeg')
                if candidate.exists():
                    return str(candidate)
            except Exception:
                pass
        ...
        def show_ffmpeg_alert():
            ...
            if venv:
                ...
                pip_path = sys.executable.replace('python.exe','Scripts/pip.exe') if os.name == 'nt' else sys.executable.replace('bin/python','bin/pip')
            if os.name == 'nt':
            ...
            win.grab_set()
            label = tk.Label(win, text="FFmpeg not found in system PATH or as Python package.\n\nPlease install ffmpeg and/or the Python package using the command below:", justify="left", wraplength=500)
            label.pack(padx=16, pady=(16,4), anchor="w")
            ...
            info = tk.Label(win, text="After installation, click OK to retry.", justify="left")
            info.pack(padx=16, pady=(0,8), anchor="w")
            ok_btn = tk.Button(win, text="OK", command=win.destroy)
        ...
        while not ffmpeg_exe:
            ...
            try:
                ...
                logger.info("ffmpeg-python is installed but ffmpeg binary not found; please install binary")
            except Exception:
                pass
            show_ffmpeg_alert()
            ffmpeg_exe = _find_ffmpeg_executable()
    ...
    # --- New block: create folder and run ffmpeg ---
        ...
        def worker(out_dir: Path, every_n_local: int, dedup_threshold: int):
            try:
                # Build a single ffmpeg invocation that applies temporal mixing
                # (tmix) over a small window and then selects every N-th frame
                # so we directly write restored sampled frames into out_dir.
                try:
                    self._processing_state = 'extracting'
                    self.after(0, lambda: self._set_status('Frame extraction started...'))
                except Exception:
                    pass
                try:
                    # vf_expr = f"tmix=frames=3,select=not(mod(n\\,{every_n_local}))"
                    ...
                    logger.info(f"Extracting restored frames (background): {' '.join(shlex.quote(str(x)) for x in ffmpeg_cmd_local)}")
                    res = subprocess.run(ffmpeg_cmd_local, capture_output=True, text=True)
                    if res.returncode != 0:
                        logger.error(f"ffmpeg error: {res.stderr}")
                        self.after(0, lambda: messagebox.showerror('ffmpeg error', res.stderr))
                        return
                except Exception as e:
                ...
                # After ffmpeg finishes, compute created frames and update status
                try:
                    out_files_local = sorted(out_dir.glob('*.png'))
                    created_count = len(out_files_local)
                    total_expected = created_count
                    try:
                        self.after(0, lambda c=created_count, t=total_expected: self._set_status(f'Frames created: {c}/{t}'))
                    except Exception:
                        pass
                except Exception:
                    created_count = 0
                ...
                if self.dedup_var.get():
                    ...
                    # initial status showing total to process
                    try:
                        self.after(0, lambda total=total_out_files: self._set_status(f'Deduplication in progress: remaining {total} files.'))
                    except Exception:
                        pass
                    # Aggressiveness threshold taken from parameter
                    DEDUP_THRESHOLD = max(0, min(32, int(dedup_threshold)))
                    for f_local in out_files_local:
                        img_local = cv2.imread(str(f_local), cv2.IMREAD_UNCHANGED)
                        ...
                        for (oh, ofp) in hashes_local:
                            if hamming(h_local, oh) <= DEDUP_THRESHOLD:
                                logger.debug(f"Removing duplicate {f_local} similar to {ofp}")
                                try:
                                    f_local.unlink()
                                    removed_local += 1
                                except Exception:
                                    pass
                                dup_local = True
                                # Request a debounced navigator update after deletion
                                try:
                                    self.after(0, lambda: self._schedule_nav_update())
                                except Exception:
                                    pass
                                # update remaining count in status bar
                                try:
                                    remaining = max(0, total_out_files - removed_local)
                                    self.after(0, lambda r=remaining: self._set_status(f'Deduplication in progress: remaining {r} files.'))
                                except Exception:
                                    pass
                                break
                        if not dup_local:
                    ...
                    try:
                        remaining = max(0, total_out_files - removed_local)
                        self.after(0, lambda rem=remaining, remd=removed_local: self._set_status(f'Deduplication completed: {remd} removed, {rem} remaining.'))
                    except Exception:
                        pass

                # Завершение: обновить навигатор и статус без модального алерта
                ...
                # Clear processing state and refresh status
                try:
                    self._processing_state = None
                    self.after(0, self._update_status)
                except Exception:
        ...
        try:
            ...
            dedup_threshold = 12
        thread = threading.Thread(target=worker, args=(output_dir, every_n, dedup_threshold), daemon=True)
        thread.start()
    def _update_file_navigator(self, force=False):
        """Обновить список PNG-файлов в навигаторе. force=True — сбросить и обновить сразу."""
        import glob
        from pathlib import Path
        ...
        current = set(self.file_listbox.get(0, tk.END))
        # Исключить parsing.json из навигатора
        new = [f.name for f in files]
        if force or set(new) != current:
            # remember currently selected filename (if any) to prefer restoring it
            try:
                cur_sel = None
                cur_idx = self.file_listbox.curselection()
                if cur_idx and len(cur_idx) > 0:
                    try:
                        cur_sel = self.file_listbox.get(cur_idx[0])
                    except Exception:
                        cur_sel = None
            except Exception:
                cur_sel = None
            ...
            self.file_listbox.delete(0, tk.END)
            for fname in new:
                if fname == 'parsing.json':
                    continue
                self.file_listbox.insert(tk.END, fname)

            # Read selected name from sidecar once as fallback (do not re-open file repeatedly)
            sel_name = None
            try:
                obj = self._get_cached_sidecar(out_dir, reload=False)
                if isinstance(obj, dict):
                    files_meta = obj.get('files', [])
                    for meta in files_meta:
                        try:
                            if meta.get('selected'):
                                sel_name = meta.get('relpath')
                                break
                        except Exception:
                            pass
            except Exception:
            ...
            # Determine which name to select: prefer previous UI selection,
            # then sidecar selection, then first item.
            file_names = [self.file_listbox.get(i) for i in range(self.file_listbox.size())]
            chosen = None
            if cur_sel and cur_sel in file_names:
                chosen = cur_sel
            elif sel_name and sel_name in file_names:
                chosen = sel_name
            elif file_names:
                chosen = file_names[0]
            ...
            if chosen:
                try:
                    ...
                    pass
            # Всегда обновлять сайдкар после выделения
            try:
                self._update_parsing_sidecar()
            except Exception:
                pass
            # If there are no files, clear preview. Otherwise update preview for current selection.
            try:
                ...
                pass
            # Refresh navigator colors to reflect sidecar
            try:
                self._refresh_nav_colors()
            except Exception:
                pass
            # Ensure status bar reflects current dataset
            try:
                self._update_status()
            except Exception:
                pass
        ...
        try:
            ...
            if not sel and self.file_listbox.size() > 0:
                try:
                    ...
                    pass
            # Debounce committing selection to parsing.json. Frequent
            # keyboard navigation should not cause atomic writes every
            # keystroke; schedule a short delayed commit instead.
            try:
                # Cancel any pending scheduled commit and perform immediate commit.
                ...
                if sel_job is not None:
                    ...
                    try:
                        ...
                        pass
                # perform immediate commit to sidecar (user requested deterministic behavior)
                try:
                    self._perform_selection_commit()
                except Exception:
                    try:
                        self._update_parsing_sidecar()
                    except Exception:
    ...
    def _on_delete_current(self, event=None):
        ...
        def _confirm_delete_dialog(filename: str):
            ...
            try:
                ...
                dlg.grab_set()
                # message
                lbl = tk.Label(dlg, text=f'Delete {filename}?', anchor='w', justify='left')
                ...
                cancel = tk.Button(btn_frame, text='Cancel', width=10, command=on_cancel)
        ...
        if not out_dir:
            return "break"
        try:
            # Cancel any pending debounced selection commit to avoid a stale
            # commit from earlier navigation overwriting our intended
            # 'selected' flag in parsing.json after deletion.
            try:
                sel_job = getattr(self, '_selection_commit_job', None)
                if sel_job is not None:
                    try:
                ...
                pass
            # Also cancel any pending navigator update job which might run
            # shortly and reset selection based on an outdated sidecar.
            try:
                nav_job = getattr(self, '_nav_update_job', None)
                if nav_job is not None:
                    try:
            ...
            # Determine next selection index BEFORE deletion so we can restore it later
            try:
                cur_idx = None
                sel_idx = self.file_listbox.curselection()
                if sel_idx and len(sel_idx) > 0:
            ...
            if target_exists:
                try:
                    ...
                    try:
                        ...
                        pass
            # remove from parsing.json entry for deleted file and compute next selection
            try:
                # list remaining files on disk (sorted)
                remaining = sorted(Path(out_dir).glob('*.png'))
                # determine desired next file name based on previous index
                if remaining:
                    if cur_idx is None:
                        desired_idx = 0
                    else:
                        desired_idx = min(cur_idx, max(0, len(remaining) - 1))
                    desired_name = remaining[desired_idx].name
                else:
                    desired_name = None
                # update sidecar: remove deleted entry and mark desired_name as selected (if any)
                try:
                    obj = self._get_cached_sidecar(out_dir, reload=False)
                    files_meta = obj.get('files', [])
                    # remove any entries matching deleted file
                    files_meta = [f for f in files_meta if f.get('relpath') != sel]
                    # clear selected flags then set for desired_name
                    for m in files_meta:
                        try:
                            if desired_name and m.get('relpath') == desired_name:
                                m['selected'] = True
                            else:
                                m.pop('selected', None)
                        except Exception:
                    ...
                    self._atomic_write_json(Path(out_dir) / 'parsing.json', obj)
                    try:
                        key = str(Path(out_dir))
                        self._parsing_sidecar_cache[key] = obj
                    except Exception:
                ...
                desired_name = None
            # refresh UI; navigator will pick up 'selected' from sidecar
            try:
                self._update_file_navigator(force=True)
            except Exception:
                pass
